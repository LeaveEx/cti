...
async def start_handler(client: Client, msg: types.Message):
    ...
    username = '@CTNMANFESSBOT' if not msg.from_user.username else '@' + msg.from_user.username
...
async def gagal_kirim_handler(client: Client, msg: types.Message):
    ...
    username = '@itsmeleave' if not msg.from_user.username else '@' + msg.from_user.username
    mention = msg.from_user.mention
